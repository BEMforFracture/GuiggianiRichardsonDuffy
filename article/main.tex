...
\begin{align*}
  \partial_{\rho}^{k}H_h(\rho, \theta) & \leq u_k\sum_{j=0}^{k}\binom{k}{j}\partial^{j}_{\rho}\left(A_{\hat x}(\rho, \theta)^{-s}\hat K_{\hat x}(\rho, \theta)\right) \\
...
Where $\displaystyle{u_k = \underset{|\bm\alpha|\leq k}{\text{max}}\left\|\partial^{\bm\alpha}\hat u\right\|_{L^{\infty}(\hat\tau)}}$ is a constant that depends only on the geometry of the element. To bound the $(m-k)$-th order derivatives of $J_h$, we used the lemma \ref{eq:lemma1}.
...
Condition \ref{eq:nondegen-lower} can be rewritten as $\boxed{r\geq c_Qh\rho}$, which represent the bi-Lipschitz property of the parametrization $\chi_h$. Furthermore, the Hadamard's inequality (HI) states that 
...
\begin{equation}
   \partial_{\rho}^{m-k}(J_h\circ\psi_{\hat x})(\rho, \theta) \leq \lambda_mh^{d-1}\sum_{j=0}^{m-k}\binom{m-k}{j} = \lambda_mh^{d-1}2^{m-k}
\end{equation}

Where $\displaystyle{\mu_m = \underset{|\bm\alpha|\leq m}{\text{max}}\left\|\partial^{\bm\alpha}\mu\right\|_{L^{\infty}(\hat\tau)}}$. We can then obtain:

\begin{equation}
  \partial^m_{\rho}G(\rho, \theta) \leq h^{d-1}\mu_{m}u_m\sum_{k=0}^{m}\sum_{j=0}^{k}2^{m-k}\binom{m}{k}\binom{k}{j}\partial^{j}_{\rho}\left(A_{\hat x}(\rho, \theta)^{-s}\hat K_{\hat x}(\rho, \theta)\right)
\end{equation}

Then, we recall the shape-regularity condition which the upper bound equivalent of the non-degeneracy condition \ref{eq:nondegen-lower}:
...
\textbf{Corollary 1:} Let $\bm f : \mathbb R\to\mathbb R^d$ be a function of class $\mathcal C^{|\bm\lambda|}$, with $\bm\lambda\in\mathbb N^d$. Then, the following bound holds (direct application of the Faà di Bruno formula with the previous lemma):

\begin{equation}
  \left|\partial^{n}\|\bm f(\rho)\|_2\right| \leq \sum_{1\leq |\bm\lambda|\leq n}\bm\lambda !C_{\bm\lambda}\|f(\rho)\|_2^{1-|\bm\lambda|}\sum_{p(n,\bm\lambda)}n!\prod_{j=1}^{n}\frac{\left(\partial^j\bm f(\rho)\right)^{\bm k_j}}{\bm k_j!j!^{|\bm k_j|}Q}
\end{equation}

\textbf{Lemma 4:} We have the following useful lower bounds: $\forall \bm k_1, \ldots, \bm k_n \in p(n, \bm\lambda)$

\begin{align}\label{ineq:lemma4}
...
with $\displaystyle{\mathcal C_n = \mathcal C_n(h, \theta, \hat x, \chi_h) = c_Q \sum_{k=1}^{n}\binom{n+k-1}{k}C_kC_{n, k}\left(\frac{e^{h\hat\rho(\theta)}}{\Gamma\left(\frac{n}{k}+2\right)}\right)^{k}}$ and $C_k = k!\left(1+\frac{1}{\sqrt 2}\right)\sqrt 2^{k}$.
...
\begin{equation*}
  \left(\frac{f}{g^s}\right)^{(n)} = \sum_{i=0}^{n}\binom{n}{i}f^{(n-i)}\sum_{k=1}^{i}(-1)^kg^{-s-k}s^{(k)}\sum_{p(i, k)}\frac{i!}{\prod_{j=1}^{i}k_j!}\prod_{j=1}^{i}\left(\frac{\partial^jg}{j!}\right)^{k_j}
\end{equation*}

Where $\displaystyle{s^{(k)} = s(s+1)\cdots(s+k-1) = \frac{(s+k-1)!}{(s-1)!}}$ is the rising factorial.

Before diving into this whole summations and product, let's establish a few straightforward bounds. Let's first write the polar equivalence (it obviously exists) of the bounding condition \ref{ineq:reg-pol-kernel}:

\begin{equation}
  \exists C_{n-i}>0, \qquad \frac{\partial^{n-i}\hat K_{\hat x}(\rho, \theta)}{\partial\rho^{n-i}} \leq \hat C_{n-i}
\end{equation}

Then, by using the mesh regularity condition \ref{eq:nondegen-lower}, we can write

\begin{equation}
  A_{\hat x}(\rho, \theta)^{-s-k} = (r/\rho)^{-s-k} \leq (c_Qh)^{-s-k}
\end{equation}

Now, our next goal is to bound the term $\displaystyle{P_{p(i, k)} = \frac{1}{\prod_{j=1}^{i}k_j!}\prod_{j=1}^{i}\left(\frac{\partial^jA_{\hat x}(\rho, \theta)}{j!}\right)^{k_j}}$. 

\begin{equation}
  P_{p(i, k)} = \prod_{j=1}^{i}\frac{\left(\partial^jA_{\hat x}(\rho, \theta)\right)^{k_j}}{j!^{k_j}k_j!} \leq \prod_{j=1}^{i}\frac{\left(h^{j+1}\mathcal C_j\right)^{k_j}}{j!^{k_j}k_j!}
\end{equation}

On one hand, $\displaystyle{\prod_{j=1}^{i}\left(h^{j+1}\mathcal C_j\right)^{k_j}} = h^{k+i}\prod_{j=1}^{i}\mathcal C_j^{k_j}$, by definition of the set $p(i, k)$. On the other hand, by lemma \ref{ineq:lemma4},

\begin{align*}
  \prod_{j=1}^{i}k_j!j!^{k_j} \geq \prod_{j=1}^{i}j!^{k_j} = \prod_{j=1}^{i}\left((j+1)!\right)^{k_j}/\prod_{j=1}^{i}(j+1)^{k_j} \overset{\ref{ineq:lemma4}}{\geq} \Gamma\left(\frac{i}{k} + 2\right)^{k}\left(\frac{i}{k}+1\right)^{-k}
\end{align*}

Thus,

\begin{equation}
  \sum_{p(i,k)}P_{p(i, k)} \leq h^{k+i}\mathcal C_{i,k}\left(\frac{\left(\frac{i}{k}+1\right)}{\Gamma\left(\frac{i}{k}+2\right)}\right)^{k}, \qquad \mathcal C_{i,k} = \sum_{p(i, k)}\prod_{j=1}^{i}\mathcal C_j^{k_j}, \qquad k \overset{\text{\tiny recall}}{=} \sum_{j=1}^{i}k_j
\end{equation}

Finally,

\begin{equation}
  \partial^k\left(A_{\hat x}(\rho, \theta)^{-s}\hat K_{\hat x}(\rho, \theta)\right) \leq \left(\frac{1}{hc_Q}\right)^s\sum_{i=0}^{k}(k)_i\hat C_{k-i}h^{i}\sum_{j=1}^{i}s^{(j)}\mathcal C_{i,j}\left(\frac{\left(\frac{i}{j}+1\right)}{\Gamma\left(\frac{i}{j}+2\right)}\right)^{j}
\end{equation}
