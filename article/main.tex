...
\textbf{Lemma 2}: if $f : \mathbb R \to E$ is a function of class $\mathcal C^{m}(E)$, then we have the following useful calculation:

\begin{equation}\label{eq:lemma2}
  \frac{\partial^n}{\partial \rho^n}\Big(\rho^sf(\rho)\Big) = \sum_{j = \max\left\{s-n, 0\right\}}^{n} \binom{n}{j}\frac{s!}{(s-n+j)!}\rho^{s-n+j}f^{(j)}(\rho)
\end{equation}

Thus, 

\begin{align}\label{eq:lemma2-applied}
  \frac{\partial^k}{\partial\rho^k}\Big(\rho^sK(\chi_h(\hat x), \chi_h\circ\psi_{\hat x}(\rho, \theta))\Big) & = \sum_{j=\max\{k-s, 0\}}^{k} \binom{k}{j}\frac{s!}{(s-k+j)!}\rho^{s-k+j}\frac{\partial^j}{\partial\rho^j}K(\chi_h(\hat x), \chi_h\circ\psi_{\hat x}(\rho, \theta))
\end{align}

Then, we notice, as well as another article (T.G. Anderson, M. Bonnet, L.M. Faria and C. Pérez-Arancibia, 2024, formula 6.27), that the partial derivatives of the kernel $K$ are bounded:

\begin{equation}\label{eq:kernel-deriv-bound}
  \left\|D_y^{\alpha}K(x, y)\right\| \leq C_K(\alpha)\left\|x-y\right\|^{-(|\alpha|+s)}, \quad\alpha\in\mathbb N^d, \quad |\alpha| = \sum_{i=1}^{d}\alpha_i
\end{equation}
