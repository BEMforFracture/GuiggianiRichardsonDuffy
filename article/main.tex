...
This is easy to understand : either look at the definition of $\widetilde w_i(\theta)$ and use the properties of $A_i(\theta)$ and $B_i(\theta)$, or integrate the constant function $F(\rho, \theta) = 1$ between $0$ and $\hat\rho(\theta)$, which is equivalent.
...
As the kernel $K$ is regularized by the term $\rho^3$ in $H_h$, we can ensure that $H_h = \mathcal O_{h\to 0} (1)$ as well as all its derivatives. Applying the Leipniz formula, we have
...
\textsc{Bounding of the $k$-th radial derivative of $J_h\circ\psi_{\hat x}$}: using the Faà di Bruno formula for functions of the form $h(x) = f[g_1(x), \cdots g_m(x)]$ (see Constantine, 1996), one can write:
...
\begin{equation}
  \exists C_Q > 0, \quad \forall h > 0, \quad \forall m>0, \quad \forall \hat x \in \hat\tau, \quad \left\|D^{m}\chi(\hat x)\right\| \leq C_Q h^{m}
\end{equation}
