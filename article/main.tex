...
Where $\displaystyle{u_k = \underset{|\bm\alpha|\leq k}{\text{max}}\left\|\partial^{\bm\alpha}\hat u\right\|_{L^{\infty}(\hat\tau)}}$ is a constant that depends only on the geometry of the element. To bound the $(m-k)$-th order polar derivatives of $J_h$, we used the lemma \ref{eq:lemma1}.
...
\begin{equation}
   \partial_{\rho}^{m-k}(J_h\circ\psi_{\hat x})(\rho, \theta) \leq \mu_mh^{d-1}\sum_{j=0}^{m-k}\binom{m-k}{j} = \lambda_mh^{d-1}2^{m-k}
\end{equation}
...
\begin{equation}
  \partial^m_{\rho}G(\rho, \theta) \leq h^{d-1}\mu_{m}u_m\sum_{j=0}^{m}\binom{m}{j}3^{m-j}\partial^{j}_{\rho}\left(A_{\hat x}(\rho, \theta)^{-s}\hat K_{\hat x}(\rho, \theta)\right)
\end{equation}
...
\textbf{Corollary 1:} Let $\bm f : \mathbb R\to\mathbb R^d$ be a function of class $\mathcal C^{|\bm\lambda|}$, with $\bm\lambda\in\mathbb N^d$. Then, the following bound holds, which is a conequence of the three following statement:
\begin{itemize}
  \item if $\bm v, \bm u\in \mathbb R^d$, then vectorial exponentitation can be simply bounded by  $\bm u^{\bm v}\leq \|\bm u\|_2^{\|\bm v\|_1}=\|\bm u\|_2^{|\bm v|}$
  ...
  \item the previous lemma \ref{eq:lemma3}
\end{itemize}
...
\textbf{Lemma 4:} We have the following useful lower and upper bounds: $\forall \bm k_1, \ldots, \bm k_n \in p(n, \bm\lambda)$
...
\begin{equation*}
  \left|\partial^n_{\rho} A_{\hat x}(\rho, \theta)\right| \leq c_Qh^{n}\sum_{1\leq |\bm\lambda| \leq n}C_{\bm\lambda}|p(n, \bm\lambda)|n!e^{|\bm\lambda|\hat\rho(\theta)h}\Gamma\left(\frac{n}{|\bm\lambda|}+2\right)^{-|\bm\lambda|}
\end{equation*}
...
\begin{equation*}
  \left|\partial^n_{\rho} A_{\hat x}(\rho, \theta)\right| \leq h^{n+1}\mathcal C_n
\end{equation*}

with $\displaystyle{\mathcal C_n = \mathcal C_n(h, \theta, \hat x, \chi_h) = c_Q \sum_{k=1}^{n}\binom{n+k-1}{k}C_kC_{n, k}\left(\frac{e^{h\hat\rho(\theta)}}{\Gamma\left(\frac{k}{n}+2\right)}\right)^{k}}$ and $C_k = k!\left(1+\frac{1}{\sqrt 2}\right)\sqrt 2^{k}$.
...
Before diving into these whole summations and product, let's establish a few straightforward bounds. Let's first write the polar equivalence (it obviously exists) of the bounding condition \ref{ineq:reg-pol-kernel}:

\begin{equation}
  \exists C_{n-i}>0, \qquad \partial^{n-i}_{\rho}\hat K_{\hat x}(\rho, \theta) \leq \hat C_{n-i} = C_{n-i}(\theta, \hat x, \chi_h)
\end{equation}
...
\begin{equation}
  P_{p(i, k)} = \prod_{j=1}^{i}\frac{\left(\partial^j_{\rho}A_{\hat x}(\rho, \theta)\right)^{k_j}}{j!^{k_j}k_j!} \leq \prod_{j=1}^{i}\frac{\left(h^{j+1}\mathcal C_j\right)^{k_j}}{j!^{k_j}k_j!}
\end{equation}
...
\begin{equation}
  \sum_{p(i,k)}P_{p(i, k)} \leq h^{k+i}\mathcal C_{i,k}\left(\frac{\frac{i}{k}+1}{\Gamma\left(\frac{i}{k}+2\right)}\right)^{k}, \qquad \mathcal C_{i,k} = \sum_{p(i, k)}\prod_{j=1}^{i}\mathcal C_j^{k_j}, \qquad k \overset{\text{\tiny recall}}{=} \sum_{j=1}^{i}k_j
\end{equation}

Finally,

\begin{equation}
  \partial^j_\rho\left(A_{\hat x}(\rho, \theta)^{-s}\hat K_{\hat x}(\rho, \theta)\right) \leq \left(\frac{1}{hc_Q}\right)^s\sum_{i=0}^{j}(j)_i\hat C_{j-i}h^{i}\sum_{k=1}^{i}s^{(k)}\mathcal C_{i,k}\left(\frac{\frac{i}{k}+1}{\Gamma\left(\frac{i}{k}+2\right)}\right)^{k}
\end{equation}
