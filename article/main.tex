...
\textit{Proof of Lemma 1:} Let $e(\rho) = G(\rho) - \Pi_{n-1}G(\rho)$ be the interpolation error of $G$ by its Lagrange interpolant $\Pi_{n-1}G$. We have the following formula for the interpolation error, which is quite general on polynomial interpolation:
...
Condition \ref{eq:nondegen-lower} can be rewritten as $\boxed{r\geq c_Qh\rho}$, which represents the bi-Lipschitz property of the parametrization $\chi_h$. Furthermore, the Hadamard's inequality (HI) states that 
...
With $\displaystyle{(k)_i = \frac{k!}{(k-i)!}}$ the falling factorial ; which gives, at the end,

\begin{align}
  \partial^m_{\rho}G(\rho, \theta) & \leq h^{d-s-1}\frac{\mu_{m}u_m}{c_Q^s}\sum_{0\leq k\leq i\leq j\leq m}h^{i}\binom{m}{j}3^{m-j}s^{(k)}(j)_i\hat C_{j-i}\mathcal C_{i,k}\left(\frac{i/k+1}{\Gamma(i/k+2)}\right)^{k} \\
  & = \underset{h\to0}{\mathcal O}\left(h^{d-s-1}\right)
\end{align}
