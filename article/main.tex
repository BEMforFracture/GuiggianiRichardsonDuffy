...
\begin{equation*}
  \left|\partial^n_{\rho} A_{\hat x}(\rho, \theta)\right| \leq c_Qh^{n}\sum_{1\leq |\bm\lambda| \leq n}C_{\bm\lambda}|p(n, \bm\lambda)|n!e^{|\bm\lambda|\hat\rho(\theta)h}P_{n, \bm\lambda}\prod_{i=1}^{n}(i+1)^{-|\bm k_i|}
\end{equation*}
...
With $\displaystyle{D_j = \sum_{0\leq k\leq i\leq j}}\frac{(j)_ik!}{j-i+1}c_Q^{-k}\mathcal C_{i,k}P_{i,k}$
