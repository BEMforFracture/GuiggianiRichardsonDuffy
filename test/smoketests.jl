...
@testset "Smoke tests - inti_integration" begin
	ops = (
		Inti.Laplace(dim = 3),
	...
	for op in ops, method in methods
		if method isa GRD.AnalyticalExpansion
			continue # Skip AnalyticalExpansion because of un-vectorized closed forms
		end
		@testset "Operator: $(typeof(op)), Method: $(typeof(method))" begin
			K = Inti.HyperSingularKernel(op)
			iop = Inti.IntegralOperator(K, Q)
			@test_nowarn GRD.adaptive_correction(iop; method = method)
			δK = GRD.adaptive_correction(iop; method = method)
			@test δK isa SparseMatrixCSC{eltype(iop), Int64}
		end
	end
end
