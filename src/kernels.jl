...
function SplitLaplaceHypersingular(qx, qy, r̂ = nothing)
	...
	nx = qx.normal
	ny = qy.normal
	return 1 / d^3, 1 / (4π) * transpose(nx) * ((I - 3 * r̂ * transpose(r̂)) * ny)
end
...
function SplitElastostaticSingleLayer(qx, qy, r̂ = nothing; μ, λ)
	...
	d = norm(r)
	r̂ = isnothing(r̂) ? r / d : r̂
	return 1 / d, 1 / (16π * μ * (1 - ν)) * ((3 - 4 * ν) * I + r̂ * transpose(r̂))
end
...
function SplitElastostaticDoubleLayer(qx, qy, r̂ = nothing; μ, λ)
	...
	r̂ = isnothing(r̂) ? r / d : r̂
	ny = qy.normal
	return 1 / d^2, -1 / (8π * (1 - ν)) * (dot(r̂, ny) * ((1 - 2 * ν) * I + 3 * r̂ * transpose(r̂)) + (1 - 2 * ν) * (r̂ * transpose(ny) - ny * transpose(r̂)))
end
...
function SplitElastostaticAdjointDoubleLayer(qx, qy, r̂ = nothing; μ, λ)
	...
	r̂ = isnothing(r̂) ? r / d : r̂
	nx = qx.normal
	return -1 / d^2, 1 / (8π * (1 - ν)) * (dot(r̂, nx) * ((1 - 2 * ν) * I + 3 * r̂ * transpose(r̂)) + (1 - 2 * ν) * (r̂ * transpose(nx) - nx * transpose(r̂)))
end
