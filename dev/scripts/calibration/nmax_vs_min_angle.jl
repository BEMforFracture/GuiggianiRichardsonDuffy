...
function _push_min_angle_point!(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, c::Case)
    ...
    if !isfinite(min_angle) || !isfinite(nmax)
        return
    end
...
function _collect_quad_groups_min_angle(op, seed::Int, nb_elements::Int, epsilon_target::Float64)
    ...
    for (i, el) in enumerate(quads)
        ...
        for config in configs
            ...
            for c in cases
                _push_min_angle_point!(groups, c)
            end
        end
    end
...
function _plot_groups_min_angle(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, title_text::String)
