...
function _push_corner_distance_point!(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, c::Case)
    ref_el = Inti.reference_domain(c.el)
    d = _distance_key(dist_to_border(ref_el, c.point))
    corner_dist = dist_to_nearest_corner(ref_el, c.point)
    nmax = float(maximum(c.n_thetas))

    if !isfinite(corner_dist) || !isfinite(nmax)
        return
    end
...
function _collect_quad_groups_corner_distance(op, seed::Int, nb_elements::Int, epsilon_target::Float64)
    ...
    for (i, el) in enumerate(quads)
        ...
        for config in configs
            ...
            for c in cases
                _push_corner_distance_point!(groups, c)
            end
        end
    end
...
function _plot_groups_corner_distance(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, title_text::String)
