...
function _push_side_ratio_point!(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, c::Case)
    ...
    if !isfinite(side_ratio) || !isfinite(nmax)
        return
    end
    ...
    push!(groups[d], (side_ratio, nmax))
end

function _collect_quad_groups_side_ratio(op, seed::Int, nb_elements::Int, epsilon_target::Float64)
    ...
    for (i, el) in enumerate(quads)
        ...
        for config in configs
            ...
            for c in cases
                _push_side_ratio_point!(groups, c)
            end
        end
    end
...
function _collect_tri_groups_side_ratio(op, seed::Int, nb_elements::Int, epsilon_target::Float64)
...
function _plot_groups_side_ratio(groups::Dict{Float64, Vector{NTuple{2, Float64}}}, title_text::String)
...
function plot_nmax_vs_side_ratio_by_distance(op, seed::Int, nb_elements::Int; epsilon_target::Float64 = 1e-10)
    quad_groups = _collect_quad_groups_side_ratio(op, seed, nb_elements, epsilon_target)
    tri_groups = _collect_tri_groups_side_ratio(op, seed, nb_elements, epsilon_target)

    fig_quad = _plot_groups_side_ratio(
        quad_groups,
        "Quadrangles - nmax vs side_ratio by distance class (epsilon=$(epsilon_target), $(nb_elements) elements)",
    )
    fig_tri = _plot_groups_side_ratio(
        tri_groups,
        "Triangles - nmax vs side_ratio by distance class (epsilon=$(epsilon_target), $(nb_elements) elements)",
    )
