...
# ============================================================
# Navier hypersingular: diagnostic Polar vs Duffy integrands
...
# Kernel setup
op = Inti.Elastostatic(; μ = mu, λ = lambda, dim = 3)
...
# -------------------------
K_polar = GRD.polar_kernel_fun(K_base, el, uhat, xhat, ori)
rho_max_fun = GRD.rho_fun(ref_domain, xhat)
L = GRD.laurents_coeffs(SK, el, ori, uhat, xhat, GRD.AutoDiffExpansion())
...
function singular_coeffs(f)
    f_m2 = f(0.0)
    f_m1 = similar(f_m2)
    for I in eachindex(f_m2)
        f_m1[I] = ForwardDiff.derivative(a -> f(a)[I], 0.0)
    end
    return f_m1, f_m2
end
...
for (k, (xiI, xiII, tau)) in enumerate(sectors)
    ...
    # Curves figure
    ax1 = Axis(fig_curves[k, 1], xlabel = "parametre normalise", ylabel = "||G1||",
        title = "Secteur $tau : G1")
    lines!(ax1, t_native, [norm(v) for v in G1p_native], color = :black, label = "Polar (theta natif)")
    lines!(ax1, bs, [norm(v) for v in G1p_composed], color = :orange, linestyle = :dash, label = "Polar compose theta(b)")
    lines!(ax1, bs, [norm(v) for v in G1d], color = :blue, label = "Duffy (b)")
    axislegend(ax1, position = :lt)

    ax2 = Axis(fig_curves[k, 2], xlabel = "parametre normalise", ylabel = "||G2||",
        title = "Secteur $tau : G2")
    lines!(ax2, t_native, [norm(v) for v in G2p_native], color = :black, label = "Polar (theta natif)")
    lines!(ax2, bs, [norm(v) for v in G2p_composed], color = :orange, linestyle = :dash, label = "Polar compose theta(b)")
    lines!(ax2, bs, [norm(v) for v in G2d], color = :blue, label = "Duffy (b)")
    axislegend(ax2, position = :lt)

    mids, dthdb = dtheta_db(thetas, bs)
    ax3 = Axis(fig_curves[k, 3], xlabel = "b", ylabel = "dtheta/db",
        title = "Secteur $tau : distorsion de parametrisation")
...
# GLMakie.save("./dev/figures/navier/navier_duffy_polar_integrand_diagnostic_curves.png", fig_curves)
# GLMakie.save("./dev/figures/navier/navier_duffy_polar_integrand_diagnostic_theta_map.png", fig_maps)
