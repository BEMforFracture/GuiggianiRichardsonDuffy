...
F̃₋₂ = GRD._old_f_minus_two_func(K, el, û, x̂)
...
first_contract = 1e-2
contract = 0.5
F̃₋₁ = GRD._old_f_minus_one_func(K_polar, rho_max_fun, F̃₋₂; first_contract = first_contract, contract = contract)
...
error = norm.(F₋₂.(θs) - F̃₋₂.(θs)) ./ norm.(F₋₂.(θs))
